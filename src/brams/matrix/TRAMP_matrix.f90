...
!!
!!-----------------------------------------------------------------------------------------------------------------------
SUBROUTINE Matrix(var,tstep,m1,noc,mech)
...
! Local variables.
      ...
      CALL Setup_Kci_dyn(var%pres(:),var%tk(:),m1) 
!----------------------------------------------------------------------------------------------------------------------
!     Begin execution.
!----------------------------------------------------------------------------------------------------------------------
      DO col=1,m1 !Begin loop in column
         ...
         PRINT *,'LFR -> No microphisics...where this routine? I did find it!'
         STOP
         return
         endif
         ...
         !----------------------------------------------------------------------------------------------------------------
         ni(seas_mode_map(:)) = var%aero(col, seas_mode_mass_map(:) ) * recip_seas_mpp(:)
         ...
         if( write_log ) then
            ...
            write(aunit1,90003)  var%pres(col),var%rh(col),var%tk(col),xh2so4_init,       xh2so4_nucl*ugm3_ncm3,so4rate,xnh3,kc,dndt,dmdt_so4
            write(aunit1,'(/a)')'var%tk(col),var%rh(col),h2so4(#.cm^3),h2so4_nucl(#/cm3),kc,nh3,dndt(#/cm3/s),ih2so4_path'      
            write(aunit1,90010)  var%tk(col),var%rh(col),ugm3_ncm3*xh2so4_init,ugm3_ncm3*xh2so4_nucl,kc,xnh3,1.0d-06*dndt,ih2so4_path
         endif
         else
         dndt     = 0.0d+00
         dmdt_so4 = 0.0d+00

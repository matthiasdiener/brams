...
!!$    use node_mod, only : mynum, nodei0, nodej0 ! DEBUG
...
!!$      !
  ...
  subroutine StoreNamelistFileAtPlume_utils(oneNamelistFile)
    implicit none
    type(namelistFile), pointer :: oneNamelistFile
    prfrq = oneNamelistFile%prfrq
  end subroutine StoreNamelistFileAtPlume_utils
...
subroutine plumerise_driver(mzp,mxp,myp,ia,iz,ja,jz,k_CO_smold,k_PM25_smold)!,block_size)
  ...
  if(mod(time+dtlt+.001,prfrq).le.dtlt.or.time.lt..01) then
...
!$OMP PARALLEL DO  
     DO i=1,nob
        ...
        DO ij=1,block_end(i)
           ...
           Tplume(i)%lpw_b(ij)=grid_g(ngrid)%lpw(indexi(ij,i),indexj(ij,i))
        END DO
     END DO
...
SUBROUTINE Lbound(imm,iveg_ag,qh,qi,qc,rsurf,plume_2d,iveg,ijbeg,ijend,alpha,&
  ...
  REAL,DIMENSION(ijbeg:ijend)   :: es
  REAL,DIMENSION(ijbeg:ijend)   :: nm1_l

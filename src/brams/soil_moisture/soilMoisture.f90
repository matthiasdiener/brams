       ...
       DO j=1,nnyp(ifm)
          DO i=1,nnxp(ifm)
             ...
             IF ( globalGlat(i,j)<latni .OR. globalGlat(i,j)>latnf .OR. &
                  globalGlon(i,j)<lonni .OR. globalGlon(i,j)>lonnf) CYCLE !GOTO 1111
             !print*,'ij=',i,j
...
!!$       ! loop no dominio local (deste processador) do modelo
!!$       DO j=1,n3
!!$          DO i=1,n2
!!$
!!$             ! evitando pontos fora do dominio da grade de umidade de solo
!!$             IF ( glat(i,j)<latni .OR. glat(i,j)>latnf .OR. &
!!$                  glon(i,j)<lonni .OR. glon(i,j)>lonnf)       CYCLE !GOTO 1111
!!$             !print*,'ij=',i,j
!!$             CALL interpolacao(glon(i,j), glat(i,j), nlon, nlat, &
!!$                  prlat, prlon, i1, i2, ic, j1, j2, jc)
!!$
!!$             print '(A,9I4)', &
!!$                  "DEBUG-ALF:soilMoistureInit:mynum,i,j,i1,i2,ic,j1,j2,jc=", &
!!$                  mynum,i,j,i1,i2,ic,j1,j2,jc
!!$             call flush(6)
!!$
!!$             IF (ic>=0 .AND. jc>=0) THEN
!!$                !print*,ic,jc,i,j,ifm
...
!!$
!!$                print '(A,5I4)', &
!!$                     "DEBUG-ALF:soilMoistureInit:mynum,qi1,qi2,qj1,qj2=", &
!!$                     mynum,qi1,qi2,qj1,qj2
!!$                call flush(6)
!!$
!!$                DO k=1,n4us
!!$                   ncount = 0
...
!!$       IF (ierr/=0) THEN
!!$          CALL fatal_error("Error allocating localChunk (soilMoistureInit)")
!!$       ENDIF
!!$       CALL CopyLocalChunk(soil_water(1,1,1,1), localChunk, &
...
!!$       IF (ierr/=0) THEN
!!$          CALL fatal_error("Error allocating gathered (soilMoistureInit)")
!!$       ENDIF
...
!!$       IF (ierr/=0) THEN
!!$          CALL fatal_error("Error allocating fullField (soilMoistureInit)")
!!$       ENDIF
!!$       varn = 'SOIL_WATER'
!!$       CALL PreProcAndGather(.FALSE., ifm, idim_type, varn, &
...
!!$
!!$       call gatherData(idim_type, varn, ifm, mzg, nnxp(ifm), nnyp(ifm), &
!!$            npat, nwave, nmachs, mchnum, mynum, master_num,             &
!!$            soil_water, Global_soil_water)
...
!!$  END SUBROUTINE gatherData4D
  ...
  SUBROUTINE apiPrlatlon(nlon, nlat, prlat, prlon, ilatn, ilonn, latni, lonni)
    ...
    REAL, INTENT(OUT)   :: prlat(:,:) !(nlon,nlat)
    REAL, INTENT(OUT)   :: prlon(:,:) !(nlon,nlat)

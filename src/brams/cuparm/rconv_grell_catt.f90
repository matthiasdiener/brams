...
!------------------------------------------------------------------------
subroutine mcphysics2(m1,m2,m3,ia,iz,ja,jz,dtlt,clsrc,rcp)
implicit none
integer m1,m2,m3,ia,iz,ja,jz,k,i,j
real dtlt
real, dimension(m1,m2,m3),intent(in ) :: clsrc
real, dimension(m1,m2,m3),intent(inout) :: rcp

do j = ja,jz
   do i = ia,iz
     do k = 1,m1
       rcp(k,i,j)=rcp(k,i,j)+clsrc(k,i,j)*dtlt
     enddo
   enddo
enddo
   ...
   do i = ia,iz
     do k = 1,m1
            ...
            if(tempk .gt. 253.)then
                 ...
                 rpp(k,i,j)=rpp(k,i,j)+clsrc(k,i,j)*dtlt
            endif
     enddo
   enddo
enddo
 ...
 forcing='ALL' 
  ...
  if(mod(time,confrq).lt.dtlt .or. time .lt. .01) then  
     ...
     if(iwork.eq.2) then
		...
		!CALL teste(mzp,mxp,myp,ia,iz,ja,jz,tend%THT(1),cuforc_sh_g(ngrid)%lsfth(1,1,1),1)
        !CALL teste(mzp,mxp,myp,ia,iz,ja,jz,tend%RTT(1),cuforc_sh_g(ngrid)%lsfRT(1,1,1),2)
...
!-srf: for only turb forcing tests
     ...
     !CALL teste(mzp,mxp,myp,ia,iz,ja,jz,tend%THT(1),cuforc_sh_g(ngrid)%lsfth(1,1,1),100)

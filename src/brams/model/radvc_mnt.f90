...
CONTAINS
  ...
  SUBROUTINE advmnt_driver(varn,m1 ,m2 ,m3 ,ia,iz,ja,jz,izu,jzv,mynum)
    !use ccattExtras            , only: extra3d,extra2d,na_EXTRA3D 
...
!
    ...
    CALL InitialFieldsUpdate(ngrids,m1,m2,m3,newm2(ngrid),newm3(ngrid),ngrid,mynum, &
     ...
     !      print *,'scal MNT=',n,scalar_tab(n,ngrid)%name; call flush(6)
     ...
     !- Aerosol sedimentation    
       ...
       current_aer_ispc = 0
       current_ndt_z    = 1
       if(ccatt == 1 .and. aerosol == 1 .and. n >= num_scalar_aer_1st) then
...
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  if(AEROSOL == 1 .and. naer_transported > 0) then 
...
!----------------------------
 ...
 SUBROUTINE advect_mnt(ngrid,m1,m2,m3,ia,iz,ja,jz,dt,mynum,n,&
  ...
  integer , parameter :: iupwind = 0
   ...
   IF(AEROSOL == 1 .and. IsThisScalarAer) then

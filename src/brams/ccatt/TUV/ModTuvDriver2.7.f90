...
!--(DMK-BRAMS-5.0-FIM)------------------------------------------------------------------
    ...
    jday = jday + nint(time/86400.)
     ...
     IF (.not. tuvDriverInit) THEN
  	...
  	if(ilwrtyp == 4 .and. iswrtyp == 4 ) then
  	...
  	CALL mclatchyTuv(1,m1,IF_adap,koff(ia,ja),jday  &
  		  ...
  		  dzl(ia,ja,2:2),1,1,rgas,g,stefan,1)
     ...
     DO j=ja,jz
  	DO i=ia,iz
  	   ...
  	   CALL mclatchyTuv(3,m1,IF_adap,koff(i,j),jday  &
  		 ...
  		 ,i,j,rgas,g,stefan,nrad(i,j))
     !print*,'4 NARAD=',narad;call flush(6)

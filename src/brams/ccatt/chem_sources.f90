...
!  END SUBROUTINE alloc_emiss_cycle
  ...
  SUBROUTINE read_sourcemaps(ng,m1,m2,m3,ia,iz,ja,jz, &
                             ...
                             rtgt,topt,transport,aer_bburn,plume_g,        &
                             tropical_forest,boreal_forest,savannah,       &
                             grassland,diur_cycle,volcanoes,volc_mean_g,   &
                             dn0,zt,zm,mchnum,master_num,mass_bin_dist,v_ash,CO2,ISFCL)
...
!    IF(next_srcfile(ng)+1 > nsrcfiles(ng)) THEN 
  ...
  SUBROUTINE read_sources_vfm(ng,m1,m2,m3,iyear,imon,iday,isrctime,fname, &
                              ...
                              mchnum,master_num,mass_bin_dist,v_ash)
    ...
    INTEGER nspecies,nxp,nyp,i,j,ii,jj
...
!    REAL, ALLOCATABLE, DIMENSION(:,:) :: src_dummy_2d
    ...
    IF (mchnum==master_num) THEN
       ...
       IF(.NOT.there) THEN
          CALL fatal_error("Source file does not exist!")
       ENDIF
       ...
       READ(iunit,*) date 
       write (*,fmt='(A)') '=== Sources Header (xpoints,ypoint,lon,lat,date) ==='
       write (*,fmt='(2(I4.4,1X),4(F8.3,1X),A)') &
    ...
    DO i=1,nspecies*nsrc*5
       ...
       IF (mchnum==master_num) THEN
          IF(TRIM(section) == 'aerosol' .AND. AEROSOL /= on) THEN
             PRINT *,'WARNING: aerosol is present but turned off in RAMSIN'
          END IF
          IF(TRIM(section) == 'plume' .AND. plumerise /= on) THEN
             PRINT *,'WARNING: plume is present but plumerise turned off in RAMSIN'
          END IF
          IF(TRIM(section) == 'volcanoes' .AND. volcanoes /= on) THEN
             PRINT *,'WARNING: volcanoes is present but turned off in RAMSIN'
          END IF
       END IF
...
!
	  ...
	  !print*, 'maxval aer=',imode, ispc,maxval(aer1_g(imode,ispc)%sc_src(1,1:m2,1:m3)),maxval(src_dummy_2d)
       ...
       ELSEIF(TRIM(section) == 'plume' .AND. plumerise == on) THEN
          ...
          !WRITE(c2,"(i8)") mchnum
          !PRINT *,'proc ',TRIM(ADJUSTL(c2)),' ->reading ',TRIM(read_mean),' for ',TRIM(read_veg_name)
...
!             CALL vfirec(iunit,plume_mean_g(iveg_agreg)%flam_frac(:,:),m2*m3,'LIN')
             ...
             WHERE(plume_mean_g(iveg_agreg)%flam_frac(:,:) < 0.) &
                   plume_mean_g(iveg_agreg)%flam_frac(:,:) = 0.
...
!             CALL vfirec(iunit,plume_mean_g(iveg_agreg)%fire_size(:,:),m2*m3,'LIN')
             ...
             WHERE(plume_mean_g(iveg_agreg)%fire_size(:,:) < 0.) &
                   plume_mean_g(iveg_agreg)%fire_size(:,:) = 0.
             ELSE
                CALL fatal_error('Model not yet prepared for flam-frac of each one of species')
          ENDIF

       !- volcanoes section  --------------------------------
       ELSEIF( ( TRIM(section) == 'volcanic-eruption' .or. TRIM(section) == 'volcanic-degassing') &
              .and. volcanoes == on) THEN
...
!             CALL vfirec(iunit,volc_mean_g%plum_heigth(:,:),m2*m3,'LIN')
             ...
             WHERE(volc_mean_g%plum_heigth(:,:)<0.)  volc_mean_g%plum_heigth(:,:)=0.
...
!             CALL vfirec(iunit,volc_mean_g%VENT_ELEV(:,:),m2*m3,'LIN')
             ...
             WHERE(volc_mean_g%vent_elev(:,:)<0.)  volc_mean_g%vent_elev(:,:)=0.
...
!             CALL vfirec(iunit,volc_mean_g%VENT_ELEV(:,:),m2*m3,'LIN')
             ...
             WHERE(volc_mean_g%duration(:,:)<0.)  volc_mean_g%duration(:,:)=0.
...
100 CONTINUE   
    ...
    IF(AEROSOL == on .and. trim(type_volc_process)=="eruption") THEN
...
END SUBROUTINE read_sources_vfm
  ...
  SUBROUTINE convert_to_tracer_density(m1,m2,m3,ia,iz,ja,jz,isrctime,      &
                                       ...
                                       aer_bburn,geoge,volcanoes,bioge,CO2,ISFCL) 
...
!            dz        = grid_g(ng)%rtgt(i,j)/dzt(ksrc) ! dzt=1/(z(k)-z(k-1))	
             ...
             DO imode=1,nmodes
                ...
                aer1_g(imode,aer_bburn)%sc_src(ksrc,i,j) = &
                aer1_g(imode,aer_bburn)%sc_src(1   ,i,j)
             ENDDO
  ...
  SUBROUTINE sources(m1,m2,m3,ia,iz,ja,jz,itime1,time,imonth1,idate1,iyear1,glon,    &
                     ...
                     emiss_cycle,volcanoes,aer_v_ash,CO2,isfcl)
...
!            *MAX(0.,radiate_g(ng)%cosz(:,:)) * (1.-f_nct) 
    ...
    DO ispc=1,chem_nspecies
	  ...
	  !- for biomass burning and volcanoes emissions          
...
!
	...
	PRINT*,' mass=',SUM(vert_mass_dist)*100.,i,j,volc_mean_g%plum_heigth(i,j),volc_mean_g%vent_elev(i,j)
	...
	DO imode=1,nmodes
	     ...
	     !- convert from kg/m^2  to  density (kg[gas]/m^3*1.e9)
	      ...
	      !  PRINT*,'ijk sc aer=',i,j,k,vert_mass_dist(k),aer1_g(imode,v_ash)%sc_src(k,i,j)

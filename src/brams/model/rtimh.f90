...
!--(DMK-CCATT-FIM)-----------------------------------------------------------
  ...
  USE DriverMatrix, ONLY: MatrixDrive  !Matrix Aerosol Model
...
!--(DMK-CCATT-FIM)--------------------------------------------------------
  ...
  CALL sfcdata  !DSM Deve-se chamar esta subrotina antes da RADIATE, caso contrario vai dar
  !              !    NAN na equacao fcpct = soil_water(mzg) / slmsts(nsoil), descrita em leaf3.f90, pois
  !              !    slmsts(nsoil) ainda nao estarah definido.
  ...
  !{DSM - Incluindo o modelo Jules
  elseif (isfcl == 4) then
     print*, "isfcl=4 -> Opcao reservada ao GEMTM. Ainda nao implementada"
     stop
  elseif (isfcl == 5) then
     if (time==0.) call SFCLYR(mzp,mxp,myp,ia,iz,ja,jz,ibcon)
...
!!$  call TimeStamp(TS_DINAMICS)
  ...
  IF(acoshdp == 0 ) THEN
     ...
     !- G3d - GD-FIM and GF
        IF(NNQPARM(ngrid)>=3) CALL CUPARM_GRELL3_CATT(OneGrid,1,NNQPARM(ngrid)) 
...
!!$  call TimeStamp(TS_DINAMICS)
  ...
  ! Aerosol Model = Matrix
  !----------------------------------------
  IF(AEROSOL==2) THEN
     CALL MatrixDrive(ia,iz,ja,jz,mzp,mxp,myp)
  END IF

...
!  imported scalars with intent(in) 
   ...
   DO l=1,land_pts
      ...
      DO n=2,npatch
         ...
         frac(l,tJ)=frac(l,tJ) + max(0.,patch_area(i,j,n)*veg_fracarea(i,j,n))
      ...
      !--- 1-veg_fracarea eh solo nu, portanto deve incluir esta fracao em tJ=8 ---
      frac(l,8)=1.-( frac(l,1)+frac(l,2)+frac(l,3)+frac(l,4)  &
                     +frac(l,5)+frac(l,6)+frac(l,7)+frac(l,9) )
                     
      if ( frac(l,8)<0. ) THEN
         PRINT*, 'Problema no calculo da fracao de solo nu. ',frac(l,8),i,j 
         STOP
      endif

      !if (i==14 .and. j==14 ) write(*,'(a,i4,9f7.3)')'l,frac(l,:)=',l,frac(l,:)
      
   ENDDO  !l=1,land_pts
  
   WHERE ( frac <= 9.99999997E-07 ) frac = 9.99999997E-07  ! este eh o valor minimo que o JULES trabalha

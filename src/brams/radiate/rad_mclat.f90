...
subroutine mclatchy(iaction,m1,if_adap,koff  &
  ...
  integer :: iaction,m1,k,lv,ls,lf,lats,latn,lat  &
       ,isummer,iwinter,is,ind,index,ilev,ifld,if_adap,koff
...
!10   CONTINUE
     ...
     if (narad /= 0) then !IF (prsnz >= 3000.) THEN
        ...
        deltap = (pl(m1-1-koff) - 1500.) / float(narad)
...
30      continue
     ...
     enddo
     zml(nrad-koff) = 2. * ztl(nrad-koff) - zml(nrad-1-koff)

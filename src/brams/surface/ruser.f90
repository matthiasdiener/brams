...
!srf----
!          soil_text(k,i,j,ipat)=6.
!srf----
	   
           nsoil = nint(soil_text(k,i,j,ipat))
	   
!              if(nint(leaf_class(i,j,2)) == 7 .or. nint(leaf_class(i,j,2)) == 8 &
!	         .or. nint(leaf_class(i,j,2)) == 15) then
!                  soil_water(k,i,j,ipat) =1.25*soil_water(k,i,j,ipat)
!test23                  soil_water(k,i,j,ipat) =1.35*soil_water(k,i,j,ipat)
!              endif


!srf - testando aumento da umidade do solo nos baixos niveis----
...
!              endif
!               if(k < 3) then
              soil_water(k,i,j,ipat) = 0.6*soil_water(k,i,j,ipat)

              soil_water(k,i,j,ipat) = max(soilcp(nsoil), &
                                       min(soil_water(k,i,j,ipat), slmsts(nsoil)))
!               else
!                soil_water(k,i,j,ipat) = 1.15*soil_water(k,i,j,ipat)
!                soil_water(k,i,j,ipat) = max(soilcp(nsoil), &
!                                         min(soil_water(k,i,j,ipat), slmsts(nsoil)))
!               endif
!srf-------------------------------------------------------------                    

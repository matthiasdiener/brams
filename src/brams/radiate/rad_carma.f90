...
!--(DMK-CCATT-INI)-----------------------------------------------------
       ,mynum,fmapt_,aot_,xland_,hrAngleLocal)
!--(DMK-CCATT-OLD)-----------------------------------------------------      
!       ,mynum,fmapt_,aot_,xland_)
!       ,mynum,fmapt_,aot_,xland_, pm_)
!--(DMK-CCATT-FIM)-----------------------------------------------------      
...
!--(DMK-CCATT-OLD)-------------------------------------------------------------------
!    REAL, POINTER, OPTIONAL :: pm_(:,:,:) !(m1,m2,m3) ! particulate material (kg[pm]/kg[air])
!--(DMK-CCATT-FIM)-------------------------------------------------------------------
...
!--(DMK-CCATT-OLD)-------------------------------------------------------------------
! !!$    REAL :: pm((iz-ia+1)*(jz-ja+1),m1)! particulate material (kg[pm]/kg[air])
!    REAL, ALLOCATABLE :: pm(:,:)! particulate material (kg[pm]/kg[air])
!--(DMK-CCATT-FIM)-------------------------------------------------------------------
...
!--(DMK-CCATT-INI)-----------------------------------------------------
    REAL :: aotldummy((iz-ia+1)*(jz-ja+1),nwave)   
!--(DMK-CCATT-FIM)-----------------------------------------------------
...
!--(DMK-CCATT-INI)-------------------------------------------------------------------
!    IF (PRESENT(pm_)) THEN
!       ALLOCATE(pm((iz-ia+1)*(jz-ja+1),m1), STAT=ierr)
!       IF (ierr/=0) CALL fatal_error("Error Allocating pm (radcarma)")
!    ENDIF
!--(DMK-CCATT-FIM)-------------------------------------------------------------------
...
!!$    !if (mynum==0) then
!!$       print '(A,I3,I4,I4,I4)', &
!!$            "DEBUG-ALF:radcarma:mynum,size(rtgt_)(rtgt)=", &
!!$            mynum, size(rtgt_,1), size(rtgt_,2), size(rtgt,1)
!!$       call flush(6)
!!$    !endif
...
!--(DMK-CCATT-OLD)-------------------------------------------------------------------
!    if (PRESENT(pm_) .AND. ALLOCATED(pm)) then !CATT==1
!!!$       CALL C_3d_2d(pm_(:,ia:iz,ja:jz),pm,m1,ia,iz,ja,jz,iend)
!      CALL C_3d_2d(pm_, pm, m1, ia, iz, ja, jz, iend)
!    endif
!--(DMK-CCATT-FIM)-------------------------------------------------------------------
...
!--(DMK-CCATT-INI)-----------------------------------------------------
    CALL Ci_3d_2d(aot_,aotldummy,nwave,ia,iz,ja,jz,iend)
!--(DMK-CCATT-OLD)-----------------------------------------------------
...
!!$      if ((t_surf(ij)*100.)<12500.) then !nlow) then
!!$         print *, "DEBUG-ALF:ij,t_surf,temprd(ij,1),rlongup,stefan=", &
!!$              ij, t_surf(ij), temprd(ij,1), rlongup(ij), stefan
!!$         call flush(6)
!!$      endif
...
!--(DMK-CCATT-INI)-------------------------------------------------------------------
      IF (cosz(ij) .gt. .003) isl_aerad(ij) = 1 !NER
!--(DMK-CCATT-OLD)-------------------------------------------------------------------
...
!print*,'xxx',n_aer,catt,aerosol,spc_alloc_aer(transport,accum,bburn),on; call flush(6)

	if (n_aer==1 ) then 
        	if(CCATT==1 .and. AEROSOL >0 .and. spc_alloc_aer(transport,accum,bburn) == ON) then
	     		CALL C_3d_2d(aer1_g(accum,bburn,ngrid)%sc_p,pm,m1,ia,iz,ja,jz,iend)
             		pmr(:,1:m1)=pm(:,1:m1)*fcui
!			pmr=pm*fcui
		else
        		pmr(1:iend,1:m1)=0.
      		endif
       	elseif (n_aer==2 ) then  
        	if(CCATT==1 .and. AEROSOL >0 .and. spc_alloc_aer(transport,accum,urban) == ON) then
	...
	elseif (n_aer==3 ) then
        	if(CCATT==1 .and. AEROSOL >0 .and. spc_alloc_aer(transport,coarse,urban) == ON) then
...
!--(DMK-CCATT-INI)-------------------------------------------------------------------
       CALL initaer(m1,pmr,dn0r,ia,iz,ja,jz,nzpmax,totm,n_aer,ntotal_aer)
!--(DMK-CCATT-OLD)-------------------------------------------------------------------
...
!--(DMK-CCATT-INI)-------------------------------------------------------------------
       CALL radtran(albedt,cosz,m1,m2,m3,ia,iz,ja,jz,aotl(:,11),n_aer,ntotal_aer)   !kml2
!--(DMK-CCATT-OLD)-------------------------------------------------------------------
...
!--(DMK-CCATT-INI)-------------------------------------------------------------------
  SUBROUTINE initaer(m1,pmr,dn0r,ia,iz,ja,jz,nzpmax,totm,n_aer,ntotal_aer)
!--(DMK-CCATT-OLD)-------------------------------------------------------------------
...
!kmlnew  
!--(DMK-CCATT-INI)----------------------------------------------------------  
  SUBROUTINE radtran(albedt,cosz,m1,m2,m3,ia,iz,ja,jz,aot11,n_aer,ntotal_aer)
!--(DMK-CCATT-OLD)----------------------------------------------------------  
...
!--(DMK-CCATT-INI)----------------------------------------------------------  
    CALL oppr(ia,iz,ja,jz,m1,aot11,n_aer,ntotal_aer)
!--(DMK-CCATT-OLD)----------------------------------------------------------  
...
!--(DMK-CCATT-INI)----------------------------------------------------------  
  SUBROUTINE oppr(ia,iz,ja,jz,m1,aot11,n_aer,ntotal_aer)
!--(DMK-CCATT-OLD)----------------------------------------------------------  
...
!--(DMK-CCATT-INI)----------------------------------------------------------  
! Expressao usada no CCATT-BRAMS 4.3.3
! Desativar para obter reprodutibilidade na temperatura e precipitacao
! da versao operacional
		idaot(ij) = n_aer !MAX(MIN(INT(10*((ANINT(10.*aot11(ij))/10.)+0.1)/2.),9),1)
!--(DMK-CCATT-OLD)----------------------------------------------------------  
! Expressao usada no BRAMS 5.0
! Ativar para obter reprodutibilidade na temperatura e precipitacao
! da versao operacional
...
!-kml-27092011         				   caer(ij,j,i,ig)
			...
			!taua(ij,l,j) = taua(ij,l,j) + (3*rdqextnew(ij,nprob(l)) * caer(ij,j,i,ig))  / &
...
!--(DMK-CCATT-INI)-------------------------------------------------------------------       
       mynum, fmapt, patch_area, npat,hrAngleLocal &
!--(DMK-CCATT-OLD)-------------------------------------------------------------------       	  
!       mynum, fmapt, patch_area, npat &
!       mynum, fmapt, patch_area, npat, pm)
!--(DMK-CCATT-FIM)-------------------------------------------------------------------       	  
       )
    !kmlnew
...
!--(DMK-CCATT-OLD)-----------------------------------------------------
!   !kmlnew
!   REAL, POINTER, OPTIONAL :: pm(:,:,:) !(m1,m2,m3) 
!--(DMK-CCATT-FIM)-----------------------------------------------------
...
!--(DMK-CCATT-INI)-----------------------------------------------------
          CALL radcarma(m1,m2,m3,ia1,iz1,ja1,jz1,solfac  &
                  ...
                  ,fmapt,carma(ngrid)%aot,xland,hrAngleLocal)
!--(DMK-CCATT-OLD)-----------------------------------------------------
!          IF (PRESENT(pm)) THEN
!             CALL radcarma(m1,m2,m3,ia1,iz1,ja1,jz1,solfac  &
!                  ,theta,pi0,pp,rv,RAIN,LWL,IWL,dn0,rtp,fthrd  &
!                  ,rtgt,f13t,f23t,glat,glon,rshort &
!                  ,rlong,albedt,cosz,rlongup,mynum  &
!                  ,fmapt,carma(ngrid)%aot,xland,pm)
!          ELSE
!             CALL radcarma(m1,m2,m3,ia1,iz1,ja1,jz1,solfac  &
!                  ,theta,pi0,pp,rv,RAIN,LWL,IWL,dn0,rtp,fthrd  &
!                  ,rtgt,f13t,f23t,glat,glon,rshort &
...
!!$    !if (mynum==0) then
!!$       print '(A,I3,I4,I4,I4)', &
!!$            "DEBUG-ALF:C_2d_1d:mynum,size(a2d)(a1d)=", &
!!$            mynum, size(a2d,1), size(a2d,2), size(a1d,1)
!!$       call flush(6)
!!$    !endif

...
module ModOneProc
  ...
  use CCattExtras, only: &
       extra2d, &
       na_extra2d , &
       StoreNamelistFileAtCCattExtras
  ...
  use aer1_list, only:           &
       ...
       mass_bin_dist,v_ash         ! (IN) read_sourcemaps()
...
contains
  ...
  subroutine OneProc(nmachs_in, mchnum_in, master_num_in)
    ...
    !--(DMK-CCATT-INI)------------------------------------------------------------------
    call StoreNamelistFileAtCCattExtras(oneNamelistFile)
    ...
    !Matrix
    CALL StoreNamelistFileAtMatrix(oneNamelistFile)
...
!- srf 21-fev-2013
!- we are trying to generalize the variable initialization procedure for meteo only
!- data and for chem + meteo data. If it works, the entire directory " isan" can be
!- deleted keeping only the directory 'isan_chem'. This will make life easier 
!- for the users.
          !if(ccatt==1 .and. chem_assim==1 .and. chemistry >= 0)then
	  ! print*, 'flags=',ccatt,chem_assim,chemistry;call flush(6);stop222
          ...
          !else    
             !--(DMK-CCATT-FIM)-----------------------------------------------------    
	  ! print*, 'flags=',ccatt,chem_assim,chemistry;call flush(6);stop224
             ...
             if (isendsrc==1) then
                do ifm = 1, ngrids
                   ...
                   call read_sourcemaps(ifm,nnzp(ifm),nodemxp(mynum,ifm),nodemyp(mynum,ifm), &
                        ...
                        mchnum, master_num,mass_bin_dist,v_ash,CO2,ISFCL)				   
                enddo
                !print*,'----------------------------------------------'
             endif
             !--(DMK-CCATT-FIM)-----------------------------------------------------

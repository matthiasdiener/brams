...
!      integer, parameter :: ensdim=16
      ...
      integer, parameter :: use_excess=2
...
!        outts,outqs,phhl
     real,    dimension (its:ite,kms:kme) ::                    &
                phh,phhl
     real,    dimension (its:ite,kts:kte) ::                    &
        SUBT,SUBQ,OUTT,OUTQ,OUTQC,subm,cupclw,dhdt,         &
        outts,outqs
!srf
...
!srf        ave_f_t,ave_f_q
     real,    dimension (its:ite,kts:kte,1:ens4) ::                    &
        omeg
     real, dimension (its:ite)            ::                    &
        ccn,Z1,PSUR,AAEQ,cuten,umean,vmean,pmean,xmbs,ZTEXC,ZQEXC
...
!tmp     if(j+j0 .ne. 138) go to 100!srf
...
!#endif
!-srf
!      if(ishallow_g3.eq.0)then
      CALL CUP_gf(xmbs,zo,outqc,j,AAEQ,T2d,Q2d,TER11,subm, &
           TN,QO,PO,PRET,P2d,OUTT,OUTQ,DT,itimestep,PSUR,US,VS, &
	   tcrit,iens,            &
           ZTEXC,ZQEXC,ccn,ccnclean,rhoi,dx,mconv, &
	   omeg, maxiens,          &
...
!      endif
!
!-srf for now shallow is not being in use - start
!
     ...
     !         Tshall,Qshall,P2d,P2d,OUTTS,OUTQS,DT,itimestep,PSUR,US,VS,    &
...
!-srf for now shallow is not being in use - end
...
!- for convective transport-end
   ...
   IMPLICIT NONE
     ...
     integer                                                           &
        ...
        its,ite, jts,jte, kts,kte,ipr,jpr,ens4, mynum
     integer, intent (in   )              ::                           &
...
!       kpbl
  ...
  !
     ...
     real, dimension (its:ite,1:ens4)                                         &
       ...
       real                                                            &
        ,intent (in   )                   ::                           &
        dx,ccnclean,dtime,tcrit,xl,cp,rv,g
       real                                                            &
        ,intent (inout   )                   ::                           &
        beta
...
!- for convective transport-start
     real,    dimension (its:ite,kts:kte) ::   berryc0_2d               
     integer, intent (in   )              ::   ccatt, mgmxp,  mgmzp   
...
!- for convective transport-end
     real mass  
      ...
      tot_time_hr=float(ktau)*dtime/3600.
      zustart=.1
      zufinal=1.
      levadj=5
      power_entr=2. ! 1.2
...
!orig cap_maxs=75.
      cap_maxs=50.
!-16042013
!srf-test capmax dep on resolution
!      cap_maxs=75.+50.*(dx/10000. -1.)
!      cap_maxs=min(cap_maxs,100.)
!
!---
...
!srf  frh=3.14*2.*radius*radius/dx/dx

!srf      if(frh .gt. 0.7)then
!srf         frh=.7
!
      if(frh .gt. 0.5)then
         frh=0.5
!srf     radius=sqrt(frh*dx*dx/(2.*3.14))
...
!
   ...
   ! Saulo's approach
            ...
            zuhe(i)=zustart
            kstart=1
!           xx1=float(kbcon(i))**power_entr
!           xx2=float(kstart)**power_entr
...
!           kbegzu=kstabi(i)+4
            kbegzu=kstabi(i)
            if(kstabi(i).lt.ktf-5)kbegzu=kstabi(i)+4
            kfinalzu=kbegzu+1
!
!srf
...
!           xx1=float(kfinalzu)**power_entr
!           xx2=float(kbegzu)**power_entr
...
!                xxx=float((k+1))**power_entr
            ...
            enddo
            do k=2,ktf-2 ! kbcon(i)
                 dz=z_cup(i,k+1)-z_cup(i,k)
              entr_rate_2d(i,k)=smth(k)/dz
            enddo
            zuhe(i)=zustart
            do k=2,kbegzu ! kbcon(i)+2
              dz=z_cup(i,k+1)-z_cup(i,k)
              frh=zuhe(i)
              zuhe(i)=zuhe(i)+entr_rate_2d(i,k)*dz*zuhe(i)-cd(i,k)*dz*zuhe(i)
!             if(zuhe(i).gt.1.)then
!               entr_rate_2d(i,k)=(1.-frh+cd(i,k)*dz*frh)/dz/frh
!               zuhe(i)=frh+entr_rate_2d(i,k)*dz*frh-cd(i,k)*dz*frh
!             endif
            enddo
         ENDIF
       enddo
...
! start from dd origin
            zd(i,jmin(i))=0.2
            zdo(i,jmin(i))=0.2
!           xx1=float(jmin(i)-levadj)**power_entr
!           xx2=float(jmin(i))**power_entr
...
!
      call cup_up_moisture('deep',ierr,zo_cup,qco,qrco,pwo,pwavo, &
           ...
           its,ite, jts,jte, kts,kte, berryc0_2d)
      do k=kts,ktf
      do i=its,itf
         cupclw(i,k)=qrco(i,k)
      enddo
      ...
      do i=its,itf
        if(ierr(i).eq.0)then
         ...
         dellaq(i,1)=(edto(i)*zdo(i,2)*qrcdo(i,2)   &
             ...
             dsubt(i,1)=0. ! (zuo(i,2)*heo_cup(i,2) &
!                    -zuo(i,1)*heo_cup(i,1))*g/dp
             dsubq(i,1)=0. ! (zuo(i,2)*qo_cup(i,2) &
!                    -zuo(i,1)*qo_cup(i,1))*g/dp
         if(j.eq.jpr.and.i.eq.ipr)then
             write(0,*)'dellas(1) '
...
!
!        dp=100.*(po_cup(i,k)-po_cup(i,k+1))
!        mass=mass+ ( dellaqc(i,k) + dellaq(i,k) )*dp/g
!	print*,'mass',k,mass,dellaqc(i,k) ,dellaq(i,k)
!     endif
!enddo;enddo
!srf ----- tmp
...
!     if(j.eq.jpr) write(0,*)'DELLAS'
      ...
      do i=its,itf
      if(ierr(i).eq.0)then
      XHKB(I)=(dsubt(i,k22(i))+DELLAH(I,K22(i)))*MBDT+HKBO(I)
      XHE(I,ktf)=HEO(I,ktf)
      XQ(I,ktf)=QO(I,ktf)
      XT(I,ktf)=TN(I,ktf)
...
!
      call cup_up_aa0(xaa0,xz,xzu,xdby,GAMMA_CUP,xt_cup, &
           ...
           itf,jtf,ktf, &
           its,ite, jts,jte, kts,kte)
!     write(66,*)'xaa0 = ',xaa0(1),aa0(1),aa1(1)
      do 200 nens=1,maxens
      do i=its,itf 
         if(ierr(i).eq.0)then
...
!       he_cup(i,1)=he(i,1)
   ...
   SUBROUTINE cup_forcing_ens_3d(closure_n,xland,aa0,aa1,xaa0,mbdt,dtime,ierr,ierr2,ierr3,&
              xf_ens,j,name,axx,tot_time_hr,maxens,iens,iedt,maxens2,maxens3,mconv,    &
...
!
       ...
       do i=its,itf
        if(ierr(i).eq.0)then
	  seed(1)=ceiling(aa0(i))
	  seed(2)=ceiling(aa1(i))
!          if(seed(1)==0) seed(1)=1+int(aa0(i))
!          if(seed(2)==0) seed(2)=1+int(aa1(i))
	  exit
        endif
       enddo
...
!
                ...
                if(irandom.eq.1)then
                   ...
                   xff_ens3(1)=max(0.,xxx*xff_ens3(1))
                   xff_ens3(4)=max(0.,xxx*xff_ens3(4))
                   xff_ens3(7)=max(0.,xxx*xff_ens3(7))
                   xff_ens3(10)=max(0.,xxx*xff_ens3(10))
                   call random_number (xxx)
                   if(xxx.lt.0.5)then
                      xxx=1.+2.*xxx
                   else
                   ...
                   xff_ens3(2)=max(0.,xxx*xff_ens3(2))
                   xff_ens3(5)=max(0.,xxx*xff_ens3(5))
                   xff_ens3(8)=max(0.,xxx*xff_ens3(8))
                   xff_ens3(11)=max(0.,xxx*xff_ens3(11))
                   call random_number (xxx)
                   if(xxx.ge.0.5)then
                      xxx=1.+xxx
                   else
                   ...
                   xff_ens3(3)=max(0.,xxx*xff_ens3(3))
                   xff_ens3(6)=max(0.,xxx*xff_ens3(6))
                   xff_ens3(9)=max(0.,xxx*xff_ens3(9))
                   xff_ens3(12)=max(0.,xxx*xff_ens3(12))
                   call random_number (xxx)
                   if(xxx.lt.0.5)then
                      xxx=1.+2.*xxx
                   else
                   ...
                   xff_ens3(13)=max(0.,xxx*xff_ens3(13))
                   xff_ens3(14)=max(0.,xxx*xff_ens3(14))
                   xff_ens3(15)=max(0.,xxx*xff_ens3(15))
                   xff_ens3(16)=max(0.,xxx*xff_ens3(16))
                endif
!  
!--- more original Arakawa-Schubert (climatologic value of aa0)
!
...
!srf                    else if(icoic.eq.0 .and. xland(i).lt.2.5)then
!                     else if(icoic.eq.0)then
!                       w_fac=1.
!                       wi_fac=1.
...
!                          wi_fac=1.5
!                     endif
!                      xf_ens(i,j,nall+1)=w_fac*xf_ens(i,j,nall+1) !/1.51
!                      xf_ens(i,j,nall+2)=w_fac*xf_ens(i,j,nall+2) !/1.51
!                      xf_ens(i,j,nall+3)=w_fac*xf_ens(i,j,nall+3) !/1.51
!                      xf_ens(i,j,nall+4)=wi_fac*xf_ens(i,j,nall+4)
!                      xf_ens(i,j,nall+5)=wi_fac*xf_ens(i,j,nall+5)
!                      xf_ens(i,j,nall+6)=wi_fac*xf_ens(i,j,nall+6)
!                      xf_ens(i,j,nall+7)=wi_fac*xf_ens(i,j,nall+7) !/1.15
!                      xf_ens(i,j,nall+8)=wi_fac*xf_ens(i,j,nall+8) !/1.15
!                      xf_ens(i,j,nall+9)=wi_fac*xf_ens(i,j,nall+9) !/1.15
!                      xf_ens(i,j,nall+10)=w_fac*xf_ens(i,j,nall+10) !*1.15
!                      xf_ens(i,j,nall+11)=w_fac*xf_ens(i,j,nall+11) !*1.15
!                      xf_ens(i,j,nall+12)=w_fac*xf_ens(i,j,nall+12) !*1.15
!                      xf_ens(i,j,nall+13)=w_fac*xf_ens(i,j,nall+3) !/1.51
!                      xf_ens(i,j,nall+14)=wi_fac*xf_ens(i,j,nall+5)
!                      xf_ens(i,j,nall+15)=wi_fac*xf_ens(i,j,nall+7) !/1.15
!                      xf_ens(i,j,nall+16)=w_fac*xf_ens(i,j,nall+10) !*1.15
!                      else if(icoic.eq.0 .and. xland(i).gt.2.5)then
!                      xf_ens(i,j,nall+1)=xf_ens(i,j,nall+1)! /1.51
!                      xf_ens(i,j,nall+2)=xf_ens(i,j,nall+2)! /1.51
!                      xf_ens(i,j,nall+3)=xf_ens(i,j,nall+3)! /1.51
!                      xf_ens(i,j,nall+4)=xf_ens(i,j,nall+1)
!                      xf_ens(i,j,nall+5)=xf_ens(i,j,nall+2)
!                      xf_ens(i,j,nall+6)=xf_ens(i,j,nall+3)
!                      xf_ens(i,j,nall+7)=xf_ens(i,j,nall+1) !/1.15
!                      xf_ens(i,j,nall+8)=xf_ens(i,j,nall+2) !/1.15
!                      xf_ens(i,j,nall+9)=xf_ens(i,j,nall+3) !/1.15
!                      xf_ens(i,j,nall+10)=xf_ens(i,j,nall+1) !/1.15
!                      xf_ens(i,j,nall+11)=xf_ens(i,j,nall+2) !/1.15
!                      xf_ens(i,j,nall+12)=xf_ens(i,j,nall+3) !/1.15
!                      xf_ens(i,j,nall+13)=xf_ens(i,j,nall+1)! /1.51
!                      xf_ens(i,j,nall+14)=xf_ens(i,j,nall+2)
!                      xf_ens(i,j,nall+15)=xf_ens(i,j,nall+3) !/1.15
!                      xf_ens(i,j,nall+16)=xf_ens(i,j,nall+4) !/1.15
!srf
                      endif
!
!
...
!
 ...
 100   continue
       ...
       if(i.eq.ipr.and.j.eq.jpr)then
             do n=1,ensdim
               write(0,*)'n,xf_ens in forcing ',n,xf_ens(i,j,n),ierr(i)
             enddo
       endif
       enddo
!     nall=0
!     write(66,*)nall,xf_ens(1,1,nall+1:nall+16)
!            do n=1,ensdim
!              write(66,*)n,xf_ens(1,1,n)
!            enddo
...
!      call massflx_stats_sa(xf_ens,ensdim,nx2,nx,maxens3,  &
...
!    weight_GR,weight_W,weight_MC,weight_ST,weight_AS, training)
       xmb_w=0.
!      call massflx_stats_sa(pr_ens,ensdim,nx2,nx,maxens3,  &
...
!
	  ...
	  if(qrc(i,k).lt.0.)then
          ...
          QC(I,K)=QRC(I,K)+qrch
          !-save for transport of tracers
	  berryc0_2d(I,K)=berryc0/(berryc+1.e-10)
...
!            ((aa1(i)-aa0(i).gt.0.) .or. (xff_shal(7).gt.0)))then
          xff_shal(4)=max(0.,-aa1(i)/(xkshal*tscl_KF))

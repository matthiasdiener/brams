...
!--(DMK-CCATT-FIM)-----------------------------------------------------
  ...
  if (isfcl == 3) then
     ...
     ! Fatal Error if using CATT or TEB-SPM with SiB
     if (CCATT==1 .or. TEB_SPM==1) then
        print*, "FATAL - It is impossible to use CATT or TEB-SPM with SiB(ISFCL=3), just with LEAF3."
        IFATERR = IFATERR + 1
     endif
  end if
...
28      format (' fatal - nested grid topography must be interpolated'  &
  ...
  do ngr=1,ngrids
     ...
     !_stc
     if(idiffk(ngr).lt.1.or.idiffk(ngr).gt.7)then
        print*,' fatal - idiffk must be 1, 2, 3, 4, 5, 6 or 7'
        ifaterr=ifaterr+1
     endif
     !srf-opt
...
if(ccatt == 1 ) then
  ...
  IF( CHEMISTRY < 0 .and. AEROSOL == 1) then
    print*,' FATAL - Cant use  AEROSOL  with CHEMISTRY < 0'
    IFATERR=IFATERR+1
  endif
...
!  endif
  IF(CHEMISTRY > 0) then
   IF( (ilwrtyp .ne. 4 .or. iswrtyp .ne. 4) .AND. trim(PhotojMethod) == 'FAST-JX')THEN
    ...
    IFATERR=IFATERR+1
   ENDIF
   IF( (ilwrtyp .ne. 4 .or. iswrtyp .ne. 4) .AND. trim(PhotojMethod) == 'FAST-TUV')THEN
    PRINT*,' FATAL  -  CARMA radiation scheme must be run with FAST-TUV WHEN CHEMISTRY  IS ON.'
    IFATERR=IFATERR+1
   ENDIF
  ...
  ELSEIF(CHEMISTRY == 0) then
	PRINT*,'Warning - chemistry 0 must be used with SPLIT_METHOD = PARALLEL'
  ...
  ![MLO - Blocking Grell deep/shallow convection without TKE
     if (nnshcu(ng) == 2 .or. NNQPARM(NG) == 2) then
        if (idiffk(ng) == 2 .or. idiffk(ng) == 3) then
           print *, 'FATAL - Grell deep and shallow cumulus requires turbulence scheme with TKE (1,4,5,6,7)'
